...
if( $id = $doctor->crear_doctor($nombre, $apellido, $sexo, $cedula, $telefono, $especialidad, 'foto') )
{
	if( isset( $doctor->mensaje ) && ! empty( $doctor->mensaje ) )
	{
		echo $doctor->mensaje;
	}
	
	$registro = $doctor->leer_doctor($id);
...
else
{
	...
	$url = 'login_datos_doctor.php?mensaje=Ya existe un usuario registrado con ese numero de cedula';
	header("Location: $url");
}
?>
<!doctype html>
...
<div id="contenido">
  ...
  <p>
    <?php 
		if( $registro )
		{
			if( isset( $registro['foto'] ) && count( $registro['foto'] ) > 0 )
		{
			echo "<img src=\"http://{$registro['foto']['url']}\" />";
		}	
